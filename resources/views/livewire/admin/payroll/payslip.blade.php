    ...
    <div class="flex flex-col sm:flex-row sm:justify-between gap-4 pb-5 border-b">
      ...
      <div class="sm:text-right">
        ...
        <p class="text-sm text-slate-500">
          {{ $payroll->period->start_date->translatedFormat('d M') }} –
  ...
  @else
    <div class="card p-5">
      <div class="flex items-center justify-between mb-4">
        ...
        <button type="button" @click="$wire.set('showItemForm', !$wire.showItemForm, true)"
        ...
        <form wire:submit="addItem" class="space-y-4">
          ...
          <div>
            ...
            <div class="flex rounded-lg border border-slate-300 overflow-hidden">
              <label
                class="flex-1 flex items-center justify-center gap-2 py-2 text-sm cursor-pointer
              ...
              <label
                class="flex-1 flex items-center justify-center gap-2 py-2 text-sm cursor-pointer border-l border-slate-300
            ...
            <div class="space-y-3">
              <div>
                ...
                <div class="relative">
                  ...
                  <span
                    class="absolute inset-y-0 right-3 flex items-center text-slate-400 text-sm font-medium">%</span>
                </div>
                @error('itemPercent')
                  <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
          ...
          <div class="flex gap-2 justify-end pt-2 border-t border-slate-100">
            <button type="button" @click="$wire.set('showItemForm', false, true)"
              class="btn-secondary">Batal</button>
            <button type="submit" class="btn-primary">Simpan Item</button>
          </div>
        </form>
      @else
        <p class="text-sm text-slate-400 text-center py-4">Klik "+ Tambah" untuk menambah bonus, tunjangan, atau
          potongan PPh 21.</p>
      @endif
    </div>
  @endif

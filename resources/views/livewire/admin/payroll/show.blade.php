<div class="space-y-4">
  <div class="flex flex-wrap items-start justify-between gap-3">
    ...
    <div class="flex flex-wrap gap-2">
      ...
      @endphp
      <span
        class="badge bg-{{ $color }}-100 text-{{ $color }}-700 capitalize self-center">{{ $period->status }}</span>
    </div>
  </div>
          ...
          @forelse ($payrolls as $pr)
            <tr class="hover:bg-slate-50">
              <td class="px-5 py-3">
                ...
                <p class="text-xs text-slate-500">{{ $pr->employee->employee_number }} ·
                  {{ $pr->employee->position?->name ?? '—' }}</p>
              </td>
              ...
              <td class="px-5 py-3 text-right">
                <a wire:navigate href="{{ route('admin.payroll.payslip', $pr) }}"
                  ...
                  <x-icon name="eye" class="w-3.5 h-3.5" /> Lihat Slip
                </a>
              </td>
            </tr>
          @empty

      ...
      <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50">
          <tr class="text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">
            <th class="px-3 sm:px-5 py-3">Pengguna</th>
            <th class="hidden sm:table-cell px-5 py-3">Peran</th>
            <th class="hidden md:table-cell px-5 py-3">Data Karyawan</th>
              ...
              <td class="px-3 sm:px-5 py-3">
                <div class="flex items-center gap-2.5">
                  ...
                  <div class="min-w-0">
                    ...
                    {{-- Mobile only: role badges --}}
                    <div class="flex flex-wrap gap-1 mt-0.5 sm:hidden">
                      @php
                        $roleColors = [
                      ...
                      @forelse ($u->getRoleObjects() as $r)
                        <span class="badge text-[10px] py-0 {{ $roleColors[$r->value] ?? 'bg-slate-100 text-slate-600' }}">{{ $r->label() }}</span>
                      @empty
              ...
              <td class="px-3 sm:px-5 py-3">
                <div class="flex items-center justify-end gap-1 sm:gap-1.5">
                  ...
                  <button type="button" @click="$wire.set('showPasswordForm', true, true); $wire.openPasswordForm({{ $u->id }})"
                    class="hidden sm:inline-flex px-2 py-1.5 rounded-lg text-xs font-medium bg-slate-50 text-slate-600 hover:bg-slate-100 transition">
                    Sandi
                  </button>
                  <button wire:click="toggleActive({{ $u->id }})"
                    class="hidden sm:inline-flex px-2 py-1.5 rounded-lg text-xs font-medium {{ $u->is_active ? 'bg-slate-50 text-slate-600 hover:bg-slate-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' }} transition">

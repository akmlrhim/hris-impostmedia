    ...
    @if ($tab === \App\Livewire\Admin\Attendance::TAB_DAILY)
        <div class="card p-4 flex flex-col sm:flex-row gap-3">
            ...
            <select wire:model.live="status" class="input md:w-52">
                ...
                @endforeach
                <option value="{{ \App\Livewire\Admin\Attendance::STATUS_MISSING }}">Tanpa Absensi</option>
            </select>
                        ...
                        @forelse ($employees as $employee)
                            ...
                            <tr class="hover:bg-slate-50" wire:key="att-{{ $employee->id }}">
                                ...
                                <td class="px-5 py-3 whitespace-nowrap">
                                    @if ($att?->status)
                                        <span
                                            class="badge bg-{{ $att->status->color() }}-100 text-{{ $att->status->color() }}-700">
                                            {{ $att->status->label() }}
                                        </span>
                                    @else
                                        @php $missing = $this->missingState($employee); @endphp
                                        <span class="badge {{ $missing['class'] }}">{{ $missing['label'] }}</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
    ...
    @else
        <div class="card p-4 space-y-3">
            ...
            <p class="text-xs text-slate-500">
                ...
                <a wire:navigate href="{{ route('admin.working-days') }}" class="text-brand-600 hover:underline">Hari
                    Kerja</a>.
            </p>
        </div>
                    ...
                    <tbody class="text-sm">
                        @forelse ($employees as $employee)
                            ...
                            <tr class="hover:bg-slate-50" wire:key="recap-{{ $employee->id }}">
                                ...
                                <td
                                    class="px-3 py-3 text-center {{ $row['present'] > 0 ? 'text-slate-700' : 'text-slate-300' }}">
                                    {{ $row['present'] }}
                                </td>
                                <td
                                    class="px-3 py-3 text-center {{ $row['late'] > 0 ? 'font-semibold text-amber-600' : 'text-slate-300' }}">
                                    {{ $row['late'] }}
                                </td>
                                <td
                                ...
                                <td
                                    class="px-3 py-3 text-center {{ $row['permission'] > 0 ? 'text-blue-600' : 'text-slate-300' }}">
                                    {{ $row['permission'] }}
                                </td>
                                <td
                                    class="px-3 py-3 text-center {{ $row['sick'] > 0 ? 'text-blue-600' : 'text-slate-300' }}">
                                    {{ $row['sick'] }}
                                </td>
                                <td

// Replace Livewire's native window.confirm with our Tailwind modal.
// Livewire 4 sets `el.__livewire_confirm` on each element with wire:confirm.
// We override that property to dispatch a window event the modal listens to.
...
function overrideConfirm(scope) {
    ...
    if (scope.nodeType === 1 && scope.hasAttribute && scope.hasAttribute('wire:confirm')) {
        apply(scope);
    }
